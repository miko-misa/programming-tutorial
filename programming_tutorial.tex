...
\section{条件分岐命令}
ある一定の法則に従ってある狭い範囲でのジャンプを行い,プロシージャの実行不実行を選択する命令を\teremp[条件分岐命令][branch instruction]という.単に分岐命令とも書く.本質的にはジャンプと大差はないがジャンプほどの自由度は設けられてないことが多く,どちらかというとプロシージャの取捨選択という意味合いの方が強い.

\subsection{\texttt{IF}文・\texttt{ELSE}文・\texttt{IF-ELSE}文}

基本的な条件分岐は\teremp[\texttt{IF}文]を使用して記述する.以下の例を見てみましょう.
